...
class MigrateFromLegacy extends Command
{
    ...
    private function step1EmailVerifiedAt(): void
    {
        ...
        $this->exec(
            ...
            fn () => DB::table('users')->where('confirmed', 1)->whereNull('email_verified_at')->update(['email_verified_at' => DB::raw('updated_at')])
        );
    }
    ...
    private function step3SpatiePermissions(): void
    {
        ...
        $this->exec(
            ...
            fn () => DB::table('roles')->where(fn ($q) => $q->whereNull('guard_name')->orWhere('guard_name', ''))->update(['guard_name' => 'web'])
        );
        ...
        // 3c. Create model_has_roles if not present
        if (!Schema::hasTable('model_has_roles')) {
            $this->warn('  model_has_roles does not exist — run php artisan migrate first.');
            return;
        }
